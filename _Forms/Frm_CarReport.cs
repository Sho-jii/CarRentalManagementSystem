...
namespace CarRentalManagementSystem._Forms
{
    public partial class Frm_CarReport : Form
    {
        ...
        public Frm_CarReport(string vehicleModel, string vehicleCondition, int timesRented, int totalDaysRented, decimal damagecost, decimal totalRevenue)
        {
            InitializeComponent();
            lblVehicleModel.Text = vehicleModel;
            lblCondition.Text = vehicleCondition;
            lblNumRented.Text = timesRented.ToString();
            lblTotalDaysR.Text = totalDaysRented.ToString();
